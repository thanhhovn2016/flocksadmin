...
} from "../../components/reactQuery/mutations/porjects";
...
const Investors = () => {
  ...
  const questionClass = questionProjectClass((state: any) => state?.question);
  ...
  const handleGetLastOrderInvestment = async () => {
    const { data } = await api.get(
      lastOrderProjectQuestion + `${questionClass?.id}/question_last_order/`
    );
    return data;
  };
  const lastOrder = useQuery(
    "lastOrderProjectQuestions",
    handleGetLastOrderInvestment
  );

  const { mutate: deleteProjectQuestion } = useMutation(
    DELETE_PROJECT_QUESTION,
    {
      onSuccess: (values: any) => {
        queryClient.invalidateQueries("projectsQuestionClassId");
  ...
  const getInvestmentQuestionFunction = async (id: number) => {
    const { data } = await api.get(
      addCompanyQuestion + `${id}/?expand=default_answer_set`
    );
    return data;
  };
  ...
  } = useMutation(UPDATE_ACTIVE_DE_ACTIVE_QUESTION_PROJECT, {
    onSuccess: (values: any) => {
      queryClient.invalidateQueries("projectsQuestionClassId")
      const status = values?.data?.status;
  ...
  const { mutate: addInvestmentQuestionFunction } = useMutation(
    ADD_COMPANY_QUESTION,
    {
      onSuccess: (values: any) => {
        ...
        setModalAddUser(!visibleModalAddUser);
        setLoadingPage(false);
        manageErrors({ code: "" }, "success", "Create Question is successfully.");
        if (status === 200) {
          // message.success(values?.data?.message);
          // history.push("/");
        } else {
  ...
  const { mutate: updateProjectQuestion } = useMutation(
    UPDATE_COMPANY_QUESTION,
    {
      onSuccess: (values: any) => {
        queryClient.invalidateQueries("projectsQuestionClassId");
  ...
  const onSubmit = (data: any) => {
    ...
    if (data?.id){
      ...
      updateProjectQuestion(variables);
    }else{
    ...
    <div style={{ position: "relative" }}>
      {loadingPage  ? <SpinLoader /> : null}
      {
        ...
        <Modal
          ...
          title={translate?.addNewQuestionProject}
          //   onOk={this.handleOk}
        ...
        <Col span={24}>
          {
            ...
            <Breadcrumb style={{ margin: "0 0", display: "flex" }}>
              ...
              }
              {
                //@ts-ignore
                <Breadcrumb.Item>
                  <Link to="/projects/Questions">
                    {translate?.projectQuestion}
                  </Link>
                </Breadcrumb.Item>
              }
              {
