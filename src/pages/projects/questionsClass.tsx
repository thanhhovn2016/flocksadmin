import {
  ...
  import { getCompanyQuestionClass, investment, investmentQuestionClass, lastOrderInvestment, lastOrderProject } from "../../components/reactQuery/constants";
...
import { ADD_INVESTMENT_QUESTION_CLASS } from "../../components/reactQuery/mutations/investment";
import { ActionTable } from "../../components/actionTable/actionTable";
import { questionClassInvestors } from "../../components/zustand/store";
import {useNavigate} from 'react-router-dom'
import { ADD_PROJECT_QUESTION_CLASS, DELETE_PROJECT_QUESTION_CLASS, UPDATE_ACTIVE_DE_ACTIVE_QUESTION_CLASS_PROJECT, UPDATE_PROJECT_QUESTION_CLASS } from "../../components/reactQuery/mutations/porjects";
import { questionProjectClass } from "../../components/zustand/store";
import DropDownComponent from "../../components/custom/dropdown";
  ...
  const Investors = () => {
    ...
    const setQuestionProjectId = questionProjectClass((state:any) => state?.setQuestionProjectId)
    ...
    const { isLoading, data, error } = useQuery("getCompanyQuestionClass", getInvestmentData ,{
      refetchOnWindowFocus: false,
      enabled:loadingPage
    } );

    const handleGetLastOrderInvestment = async () => {
        const {data} = await api.get(lastOrderProject)
        return data
    }
    ...
    const {
      mutate: deleteProjectQuestionClass,
    } = useMutation(DELETE_PROJECT_QUESTION_CLASS, {
      onSuccess: (values: any) => {
        queryClient.invalidateQueries("getCompanyQuestionClass")
    ...
    const handleActionTable = (row:any , actionType:string) => {
        if (actionType === "questions"){
            ...
            setQuestionProjectId(row)
            navigation('/projects/questionDetails')
        }
        ...
        if (actionType === "Delete"){
          deleteProjectQuestionClass(row?.id)
          setLoadingPage(true);
    ...
    } = useMutation(UPDATE_ACTIVE_DE_ACTIVE_QUESTION_CLASS_PROJECT, {
      onSuccess: (values: any) => {
        queryClient.invalidateQueries("getCompanyQuestionClass")
        const status = values?.data?.status;
      ...
      },
   {
    ...
    const handelOpenModalUserManagement = () => {
      ...
      } = useMutation(ADD_PROJECT_QUESTION_CLASS, {
        onSuccess: (values: any) => {
          queryClient.invalidateQueries("getCompanyQuestionClass")
          const status = values?.data?.status;
      ...
      const {
        mutate: updateProjectQuestionClass,
      } = useMutation(UPDATE_PROJECT_QUESTION_CLASS, {
        onSuccess: (values: any) => {
          queryClient.invalidateQueries("getCompanyQuestionClass")
          const status = values?.data?.status;
      ...
      const onSubmit = (data: any) => {
        ...
        const   newVariables:any = {
          ...
          updateProjectQuestionClass(newVariables)
        }else{
    ...
    return (
      <div style={{position:"relative"}}>
        ...
        title={translate?.addNewClassNameProjectTitle}
        //   onOk={this.handleOk}
        ...
        <Col span={24}>
          {
            ...
            {
              //@ts-ignore
              <Breadcrumb.Item>{translate?.projectQuestion}</Breadcrumb.Item>}
          </Breadcrumb>}
